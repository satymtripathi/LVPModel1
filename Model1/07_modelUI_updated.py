# ============================================================
# Streamlit UI (UPDATED) for:
# SAFE + DEVICE-ROBUST Dual-Branch MIL + Clinical Tile Scoring
# + Soft Attention Pooling + Tabular Feature Standardization
#
# ✅ Matches your latest TRAINING (V5_doctorlike):
...
#
# NOTE:
# - We do NOT need meta.json at inference.
# - No overdoing: we only changed what training requires.
# ============================================================
...
# =========================
# Clinical Tile Scoring (MATCH TRAINING V5)
# =========================
import math
...
# =========================
# Feature extraction (MATCH TRAINING V5)
# =========================
...
def aggregate_tile_features(tile_dicts: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    mat = np.stack([dict_to_vec(td, keys) for td in tile_dicts], axis=0)
...
def compute_feature_vector(global_rgb: np.ndarray, tiles_sorted_by_quality: List[np.ndarray], feat_keys: List[str]) -> np.ndarray:
    feats_global = extract_feature_vector_from_rgb(global_rgb)
    feats = {f"g_{k}": v for k, v in feats_global.items()}

    if cfg.TOP_TILES_FOR_FEATURES > 0 and len(tiles_sorted_by_quality) > 0:
        top_tiles = tiles_sorted_by_quality[:max(1, min(cfg.TOP_TILES_FOR_FEATURES, len(tiles_sorted_by_quality)))]
        tile_dicts = [extract_feature_vector_from_rgb(t) for t in top_tiles]
        feats.update(aggregate_tile_features(tile_dicts))

    return dict_to_vec(feats, feat_keys)

# =========================
# Polar tiles (coords in full 512)
# =========================
...
# =========================
# MODEL (UPDATED: Soft attention pooling + valid mask)
# =========================
...
# =========================
# Loading models + feature meta (keys + mu/sigma)
# =========================
@st.cache_resource
def load_everything():
    # feature meta
    ...
    if not os.path.exists(cfg.FEAT_MU_NPY):
        raise FileNotFoundError(f"feat_mu.npy not found: {cfg.FEAT_MU_NPY}")
    if not os.path.exists(cfg.FEAT_SIGMA_NPY):
        raise FileNotFoundError(f"feat_sigma.npy not found: {cfg.FEAT_SIGMA_NPY}")
    ...
    model = DualBranchMIL_WithFeatures(
    ...
    return seg_model, idx_crop, idx_limbus, img_size, model, classes, feat_keys, feat_mu, feat_sigma, topk_pool, quality_beta
...
try:
    with st.spinner("Initializing Deep Learning Engine..."):
        seg_model, idx_crop, idx_limbus, img_size, clf_model, CLASSES, FEAT_KEYS, FEAT_MU, FEAT_SIGMA, CK_TOPK, CK_QB = load_everything()
...
topk_val = st.sidebar.slider("Attention Top-K (display only)", 1, 12, int(CK_TOPK))
...
fast_mode = st.sidebar.toggle("Fast Mode (skip tile features)", value=False)
if fast_mode:
...
# ---- TILE EXTRACTION on full 512 masked canvas ----
...
# pad to 24 (KEEP SORT ORDER)
tiles_pad = tiles_sorted[:cfg.MAX_TILES]
q_pad_raw = q_sorted[:cfg.MAX_TILES]
coords_pad = coords_sorted[:cfg.MAX_TILES]

pad_n = max(0, cfg.MAX_TILES - len(tiles_pad))
if pad_n > 0:
    tiles_pad += [np.zeros((cfg.TILE_SIZE, cfg.TILE_SIZE, 3), dtype=np.uint8)] * pad_n
    q_pad_raw = np.concatenate([q_pad_raw, np.zeros((pad_n,), dtype=np.float32)], axis=0)
    coords_pad += [None] * pad_n

# qnorm (0..1) computed only on real tiles; padded = 0
q01 = np.zeros((cfg.MAX_TILES,), dtype=np.float32)
if len(q_sorted) > 0:
    q_real = q_sorted[:min(len(q_sorted), cfg.MAX_TILES)]
    q_real01 = normalize01(q_real)
    q01[:len(q_real01)] = q_real01.astype(np.float32)

valid_mask = np.zeros((cfg.MAX_TILES,), dtype=np.float32)
valid_mask[:min(len(q_sorted), cfg.MAX_TILES)] = 1.0
...
# features:
# - global features from the same global we use for model
# - tile features from quality-sorted REAL tiles (not padded)
feat_vec = compute_feature_vector(global_rgb_for_model, tiles_sorted, FEAT_KEYS).astype(np.float32)
feat_vec = (feat_vec - FEAT_MU) / FEAT_SIGMA
f_t = torch.from_numpy(feat_vec).unsqueeze(0).to(DEVICE)

# runtime params
clf_model.topk_pool = int(topk_val)          # display top-k only
...
with st.spinner("Running MIL diagnosis..."):
    with torch.no_grad():
        with autocast(enabled=(DEVICE == "cuda")):
            logits, att, top_idx = clf_model(g_t, t_t, f_t, qnorm=q_t, valid_mask=vm_t)
...
with col_main:
    ...
    tabs[1].image(global_rgb_for_model, use_container_width=True, caption="Global ROI (cropped bbox of crop-mask)")
